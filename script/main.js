...
class TodoListDom extends TodoList {
  constructor() {
    super();
  ...
  #getTodoItemTemplate(todoItem) {
    return `<li class="list__item ${todoItem?.done ? 'done' : ''} ${
      todoItem?.isNew ? 'new' : ''
    }" data-id="${todoItem.id}"><span class="list__text">${todoItem.text}</span>
      <label class="list__checkbox-label"><input class="list__checkbox-input" type="checkbox" ${
        todoItem.done ? 'checked' : ''
      } tabindex="0"><span class="list__checkbox-button"><svg class="list__checkbox-icon" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  render() {
    this.$todoList.innerHTML = this.data
      .map(this.#getTodoItemTemplate)
      .join('');
  }

  #getListElementButtons() {
    // дістаю елементи коли створиться елемент списку (спрацює функція #onFormSubmit)
